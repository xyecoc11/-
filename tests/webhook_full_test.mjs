...
if (!process.env.SUPABASE_URL || (!process.env.SUPABASE_SERVICE_ROLE_KEY && !process.env.SUPABASE_ANON_KEY)) {
  console.error('[Test] Missing SUPABASE_URL or keys (need SERVICE_ROLE_KEY or ANON_KEY)');
  process.exit(1);
}
...
async function sendWebhook(type, data) {
  const body = { id: `evt_${type}_${Date.now()}`, type, data };
...
async function checkTable(table, filter) {
