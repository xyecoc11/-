...
export async function GET(request: Request) {
  // Check SYNC_TOKEN from Authorization header
  ...
  if (!expectedToken) {
    console.error('[Whop Sync] SYNC_TOKEN not configured');
  ...
  if (!providedToken || providedToken !== expectedToken) {
    console.warn('[Whop Sync] Invalid or missing token');
    return NextResponse.json(
      { error: 'Unauthorized' },
      { status: 401 }
    );
  ...
  try {
    console.info('[Whop Sync] Starting sync...');

    // Fetch all data sequentially with logs
    console.info('[Whop Sync] Fetching users...');
    const whopUsers = await fetchWhopUsers();

    console.info('[Whop Sync] Fetching memberships...');
    const whopSubs = await fetchWhopSubscriptions();

    console.info('[Whop Sync] Fetching orders...');
    const whopOrders = await fetchWhopOrders();

    console.info('[Whop Sync] Fetching refunds...');
    const whopRefunds = await fetchWhopRefunds();
